...
print_streams(struct Stream * s, int starttime, int endtime, int printmode) {
...
make_adjusted_profile(int prev_p, int h, struct Snote * sn) {
...
meter_stats() {
    ...
    for(t=num_tactus_beats-3; t--; t>=0) {
	if(!pip[tp[t]].hasnote) {
	    ntb--;
	    lastt = t;
	}
